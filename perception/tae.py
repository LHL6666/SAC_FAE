...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, obs_shape, patch_size=4, num_filters=96, norm_layer=None):
        super().__init__()
        ...
        self.proj = nn.Conv2d(obs_shape[0], num_filters, kernel_size=patch_size, stride=patch_size)
        if norm_layer is not None:
            self.norm = norm_layer(num_filters)
        else:
            self.norm = None
    ...
    def flops(self):
        ...
        flops = Ho * Wo * self.num_filters * self.in_channels * (self.patch_size[0] * self.patch_size[1])

        if self.norm is not None:
            flops += Ho * Wo * self.num_filters
...
class BasicLayer(nn.Module):
    ...
    Args:
        num_filters (int): Number of input channels.
    ...
    def __init__(self, num_filters, input_resolution, depth, num_heads, window_size,
        ...
        if downsample is not None:
            self.downsample = downsample(input_resolution, dim=num_filters, norm_layer=norm_layer)
        else:
    ...
    def extra_repr(self) -> str:
        return f"num_filters={self.num_filters}, input_resolution={self.input_resolution}, depth={self.depth}"
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, num_filters, input_resolution, depth, num_heads, window_size,
        ...
        if upsample is not None:
            self.upsample = PatchExpand(input_resolution, dim=num_filters, dim_scale=2, norm_layer=norm_layer)
        else:
            self.upsample = None
...
class TransformerAutoencoder(nn.Module):
    ...
    def __init__(self, obs_shape, feature_dim=300, num_filters=128, num_layers=None, batch_vel_acc=None,
        ...
        self.num_channels_up = int(num_filters * 2)
        ...
        self.encoder_latent = nn.Linear(in_features=256 * 96, out_features=self.feature_dim, bias=True)
        ...
        self.decoder_latent = nn.Linear(in_features=self.feature_dim, out_features=256 * 96, bias=True)
        for i_layer in range(self.num_layers):
            if i_layer == 0:
            ...
            self.decoder.append(layer_up)

        print("encoder block: ", self.encoder)
        print('\n')
        print("decoder block: ", self.decoder)
    ...
    def forward_decoder(self, x):
        for inx, layer_up in enumerate(self.decoder):
            x = layer_up(x)
    ...
    def forward(self, x):
        ...
        print("forward latent shape: ", latent_var.shape)

        b, hw, c = latent_var.shape

        latent_var = latent_var.view(b, -1)
        print("encoder latent shape: ", latent_var.shape)
        latent_var = self.encoder_latent(latent_var)

        print("latent shape: ", latent_var.shape)

        latent_var = self.decoder_latent(latent_var)
        print("decoder latent shape: ", latent_var.shape)
        latent_var = latent_var.view(b, hw, c)

        rec_features = self.forward_decoder(latent_var)
        rec_image = self.decoder_output(rec_features)
    ...
    def flops(self):
        ...
        for i, layer in enumerate(self.encoder):
            flops += layer.flops()
        flops += self.num_channels * self.patches_resolution[0] * self.patches_resolution[1] // (2 ** self.num_layers)
        flops += self.num_channels * self.num_filters
...
def test_tae():
    obs_shape = (3, 128, 128)
    tae = TransformerAutoencoder(obs_shape=obs_shape, feature_dim=384, num_filters=48, num_layers=2, patch_size=4,
                                 window_size=8)
    ...
    print("Network parameters: ", torchsummary.summary(tae.cuda(), (3, 128, 128)))
